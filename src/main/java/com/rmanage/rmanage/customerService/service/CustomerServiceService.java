...
public class CustomerServiceService {
    ...
    public CustomerServiceResponseDto saveCustomerServiceById(CustomerServiceRequestDto customerServiceRequestDto) {
        ...
        try {
            ...
            if(entity.isEmpty()){
                return new CustomerServiceResponseDto(false,3002,"해당하는 근로자 정보가 없음",null);
            }
            User user = entity.get();
            // 조회 성공
            customerServiceRespository.save(customerServiceRequestDto.CustomerServicetoEntity(user));

            return new CustomerServiceResponseDto(true,1056,"고객센터 문의 성공", null);
        }   catch (Exception e){
            System.out.println(e);
            return new CustomerServiceResponseDto(false,3065,"고객센터 문의 실패",null);
        }
